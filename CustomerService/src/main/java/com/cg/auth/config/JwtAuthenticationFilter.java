...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    private final List<String> AUTH_WHITELIST = List.of("/actuator/**","/auths/**", "/v3/api-docs/**", "/swagger-ui/**", "/swagger-ui.html");
        ...
        if (authorizationHeader != null && authorizationHeader.startsWith("Bearer ")) {
            ...
            if (!jwtUtil.isTokenExpired(token)) {
                ...
                if (username != null && SecurityContextHolder.getContext().getAuthentication() == null) {
                    ...
                    throw new AuthServiceException("Invalid JWT token", 401);
                }
            } else {
                throw new AuthServiceException("JWT token is expired", 401);
            }
        } else {
            // Commented out exception handling for missing token
             throw new AuthServiceException("Authorization JWT Token is missing in Header", 401);
        }
        chain.doFilter(request, response);
    }
